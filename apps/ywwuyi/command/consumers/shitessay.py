...
famous_list = words["famous"] # a 代表前面垫话，b代表后面垫话
before_list = words["before"] # 在名人名言前面弄点废话
after_list = words['after']  # 在名人名言后面弄点废话
bosh_list = words['bosh'] # 代表文章主要废话来源
...
def shuffle_traverse(l):
    global repeatability
    pond = list(l) * repeatability
    while True:
        random.shuffle(pond)
        for element in pond:
            yield element
...
next_bosh = shuffle_traverse(bosh_list)
next_famous = shuffle_traverse(famous_list)


def get_famous():
    global next_famous
    xx = next(next_famous)
    xx = xx.replace("a", random.choice(before_list))
    xx = xx.replace("b", random.choice(after_list))
    return xx
...
def get_shit_essay(data):
    group_id = str(data["group_id"])
    message = str(data["message"])
    keyword = message.replace("如何评价", "", 1).replace("?", "").replace("？", "")
    if keyword == "鸡哥":
        msg = "鸡哥相信大家都很熟悉，鸡哥其实就是一个憨批"
        send_group_msg(msg, group_id)
    else:
        msg = "谢邀，人在飞机，刚下美国，圈内熟人太多，先匿了\r\n      "
        # for i in range(random.randint(3, 6)):
        count = 0
        while len(msg) < 1000:
            r = random.randint(0, 100)
            if r < 5 and count > 1 or count >= 8:
                msg += new_part()
                count = 0
                # part_len = 0
            elif r < 20:
                ...
                msg += _famous
                count += 1
                # part_len += len(_famous)
            else:
                _bosh = next(next_bosh)
                msg += _bosh
                count += 1
                # part_len += len(_bosh)
            # print(msg)
            # print(part_len)
        msg = msg.replace("x", keyword)
